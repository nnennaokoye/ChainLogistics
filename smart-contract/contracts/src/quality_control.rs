...
use crate::storage;
use crate::types::{Product, QualityReading, QualityCertification, QualityParameter};
...
impl QualityControlContract {
    ...
    pub fn init_quality_control(
        ...
        env.events().publish(
            (Symbol::new(&env, "quality_control_initialized"), product_id.clone()),
            admin,
        );
    ...
    pub fn record_quality_reading(
        ...
        let status = Self::determine_reading_status(
            &env,
            value,
            threshold_min,
            threshold_max,
        ...
        // Emit event
        env.events().publish(
            (Symbol::new(&env, "quality_reading_recorded"), product_id.clone()),
            (sensor_id, parameter, value, unit, status),
        );
        
        // Trigger alert if out of range
        if status == String::from_str(&env, "critical") {
            env.events().publish(
                (Symbol::new(&env, "quality_alert_triggered"), product_id.clone()),
                (parameter, value, String::from_str(&env, "threshold_exceeded")),
            );
        }
    ...
    pub fn define_quality_parameters(
        ...
        env.events().publish(
            (Symbol::new(&env, "quality_parameters_defined"), product_id.clone()),
            parameters.len(),
        );
    ...
    pub fn get_quality_summary(
        ...
        summary.set(String::from_str(&env, "total_readings"), format_count(&env, total_readings));
        summary.set(String::from_str(&env, "critical_count"), format_count(&env, critical_count));
        summary.set(String::from_str(&env, "warning_count"), format_count(&env, warning_count));
        summary.set(String::from_str(&env, "normal_count"), format_count(&env, normal_count));
        
        let certifications = storage::get_quality_certifications(&env, &product_id);
        summary.set(String::from_str(&env, "certification_count"), format_count(&env, certifications.len()));
    ...
    fn generate_reading_id(
        ...
        // Build reading ID by concatenating product_id, sensor_id, and timestamp
        let mut id_str = String::new(env);
        id_str.push_str(product_id);
        id_str.push_str(&String::from_str(env, ":"));
        id_str.push_str(sensor_id);
        id_str.push_str(&String::from_str(env, ":"));
        id_str.push_str(&Self::u64_to_string(env, timestamp));
        id_str
    }
    ...
    fn u64_to_string(env: &Env, n: u64) -> String {
        ...
        let mut num = n;
        let mut digits: Vec<u8> = Vec::new(env);
        
        while num > 0 {
            digits.push_back((num % 10) as u8 + b'0');
            num /= 10;
        }
        ...
        let len = digits.len();
        for i in (0..len).rev() {
            let digit_char = core::char::from_u32(digits.get(i).unwrap_or(b'0') as u32).unwrap_or('0');
...
fn format_count(env: &Env, count: u32) -> String {
    ...
    if n == 0 {
        ...
        while n > 0 {
            digits.push_back((n % 10) as u8 + b'0');
            n /= 10;
        }
    }
    ...
    for i in (0..digits.len()).rev() {
        if let Some(digit) = digits.get(i) {
            let digit_char = core::char::from_u32(digit as u32).unwrap_or('0');
            result.push(digit_char);
        }
    }
